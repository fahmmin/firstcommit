...
/* ── storefront ────────────────────────────────────────────── */
// Public product catalog — built by the digital-presence agent, shared with buyers.
const MP_LABEL = {
  facebook_marketplace: 'Facebook Marketplace', indiamart: 'IndiaMART',
  shopify: 'Shopify', instagram: 'Instagram Shop',
}
function Storefront({ data = {} }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between">
        <div>
          <div className="text-[11px] text-slate-400 uppercase tracking-wide">Product catalog</div>
          <div className="text-[18px] font-semibold text-ink">{data.business || '—'}</div>
          {data.tagline && <div className="text-[11px] text-slate-500 mt-0.5">{data.tagline}</div>}
        </div>
        <Store size={22} className="text-slate-300" />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        {products.map((p, i) => (
          <div key={i} className="rounded-xl border border-slate-200 bg-white p-3">
            <div className="text-[13px] font-medium text-ink leading-snug">{p.title}</div>
            {p.desc && <div className="text-[11px] text-slate-500 mt-1 line-clamp-2">{p.desc}</div>}
            <div className="mt-2 flex items-center justify-between">
              <span className="text-[14px] font-semibold text-ink">{fmt(p.price)}
                <span className="text-[10px] font-normal text-slate-400">/{p.unit || 'unit'}</span>
              </span>
              {p.category && <span className="text-[9px] text-slate-400 uppercase tracking-wide">{p.category}</span>}
            </div>
          </div>
        ))}
      </div>
      ...
      {(data.marketplaces || []).length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {data.marketplaces.map((m) => (
            <span key={m} className="text-[10px] px-2 py-1 rounded-full bg-slate-100 text-slate-600">
              Also on {MP_LABEL[m] || m}
            </span>
          ))}
        </div>
      )}

      <div className="rounded-2xl bg-ink text-white p-4 flex items-center justify-between">
        <div className="text-[12px] text-white/70">{data.note || 'Bulk rates on request.'}</div>
        {data.contact && <div className="text-[12px] font-medium">{data.contact}</div>}
      </div>
      <p className="text-[10px] text-slate-400">Catalog generated by Sahayak AI on behalf of the seller.</p>
    </div>
  )
}
